...
# setwd('<dir where rds files are>')
triGramTable2 <- readRDS('triGramTable2.rds')
biGramTable2  <- readRDS('biGramTable2.rds')
uniGramTable2 <- readRDS('uniGramTable2.rds')


# Extend n-gram tables to include discount ----------------------------------------------------

####
# Important assumption: This (example) is for 3-gram table. Do similarly for 2-gram or 4-gram table to have "discount" column and leftover_probability.
...
createNGramTableExtended = function(dt){
  ...
  for(i in 5:1){
    ...
    currN = nrow(dt[frequency == currRTimes])
    nextN = nrow(dt[frequency == nextRTimes])
    
    currd = (nextRTimes / currRTimes) * (nextN / currN) # assumption: 0 < d < 1
    
    # the beauty of "data.table"!
    dt[frequency == currRTimes, discount := currd]
  }
  return(dt)
}

threeGramTable <- createNGramTableExtended(triGramTable2)
twoGramTable   <- createNGramTableExtended(biGramTable2)
oneGramTable   <- createNGramTableExtended(uniGramTable2)


# Distribute "leftover" probability after discounting -----------------------------------------
...
# Utility functions to prepare strings --------------------------------------------------------
