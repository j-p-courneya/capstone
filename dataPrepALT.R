...
# 
# blogs <- readLines(conB,encoding='UTF-8',skipNul=TRUE)
# news <- readLines(conN,encoding='UTF-8',skipNul=TRUE)
# twitter <- readLines(conT,encoding='UTF-8',skipNul=TRUE)
# 
# close(conB)
# close(conN)
# close(conT)
...
n <- c(length(blogs),length(news),length(twitter))
...
preProcess <- function(idx) {
  ...
  corpus <- lapply(c(blogs,news,twitter)[resample[[idx]]],
