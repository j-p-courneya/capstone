...
Using `readLines` the text files are imported as character vectors.
...
Find out the size of the files, # of lines, and words in each.
...
Visualize the lines, characters, and words in the documents
...
At this stage, it is not uncommon to remove stopwords - since the purpose of this project is to create a predictive text model, we will keep them. We filter for profane language, using a list of words available at [LDNOOBW](https://github.com/LDNOOBW/List-of-Dirty-Naughty-Obscene-and-Otherwise-Bad-Words/blob/master/LICENSE) the profane words will be removed during building the `r dfm`
...
Work with a sample corpus to accomodate for insufficient processing performance in my local computing environment.
```{r}
...
twitter_sample <- corpus(twitter_sample)
```

```{r}
blogtokens <- tokens(blogs_sample, what = "fasterword", remove_punct  )
tokens_ngrams(blogs_sample, n=2L)
```
