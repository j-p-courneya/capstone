...
```{r}
uniGramTable <- data.table(freqUni$feature, freqUni$frequency)
names(uniGramTable) <- c("ngram", "frequency")
biGramTable <- data.table(freqBi$feature, freqBi$frequency)
names(biGramTable) <- c("ngram", "frequency")
triGramTable <- data.table(freqTri$feature, freqTri$frequency)
names(triGramTable) <- c("ngram", "frequency")
```
...
trigramList <- separateTerms(triGramTable$ngram)
triGramTable2 <- data.table(trigramList$firstTerms, trigramList$lastTerm, triGramTable$frequency)
names(triGramTable2) <- c("firstTerms", "lastTerm", "frequency")
# create separate terms for 2-grams
bigramList <- separateTerms(biGramTable$ngram)
biGramTable2 <- data.table(bigramList$firstTerms, bigramList$lastTerm, biGramTable$frequency)
names(biGramTable2) <- c("firstTerms", "lastTerm", "frequency")
# create 1-gram table similar to [2,3]-gram tables
uniGramTable2 <- data.table(uniGramTable$ngram, uniGramTable$frequency)
names(uniGramTable2) <- c("lastTerm", "frequency")
```
